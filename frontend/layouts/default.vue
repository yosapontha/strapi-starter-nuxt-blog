        ...
        <div class="uk-navbar-right">
          <ul class="uk-navbar-nav">
              <li v-bind:key="category.id" v-for="category in categories">
                <router-link :to="{ name: 'categories-slug', params: { id: category.id, slug: category.slug }}" tag="a">{{ category.name }}
                </router-link>
              </li>
          </ul>
        </div>
                ...
                <div class="uk-padding-large">
                    ...
                    <div class="uk-width-1-2@s">
                        <ul class="uk-nav-primary uk-nav-parent-icon" uk-nav>
                          <li v-bind:key="category.id" v-for="category in categories">
                            <router-link class="uk-modal-close" :to="{ name: 'categories-slug', params: { id: category.id, slug: category.slug }}" tag="a">{{ category.name }}
                            </router-link>
                          </li>
                        </ul>
                    </div>
